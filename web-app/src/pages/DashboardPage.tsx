...
function BatteryIcon({ percent, charging }: { percent: number; charging: boolean }) {
  ...
  return (
    <div className="relative flex h-6 w-10 items-center">
      <div className="h-5 w-9 rounded border-2 border-gray-500">
        <div className="h-full rounded-sm transition-all" style={{ width: `${percent}%`, backgroundColor: fill }} />
      </div>
      <div className="absolute -right-1 h-2 w-1.5 rounded-r bg-gray-500" />
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Card>
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[9px] font-bold text-gray-500 uppercase tracking-widest">Signal</p>
              <p className={`mt-1 text-2xl font-bold ${signalColor(pccRsrp)}`}>
                {pccRsrp != null ? `${pccRsrp}` : '\u2014'}
              </p>
              <p className="text-xs text-gray-500">dBm RSRP</p>
            </div>
          ...
          <div className="mt-2 flex items-center gap-1.5">
            <span className={`rounded-lg px-2 py-0.5 text-[9px] uppercase font-bold border shadow-sm ${
              isNR ? 'bg-purple-100 text-purple-700 border-purple-200' : 'bg-slds-blue/10 text-slds-blueHover border-slds-blue/30'
            }`}>
              {signal?.type ?? '\u2014'}
            </span>
            <span className="text-xs text-gray-500">{signal?.band ?? ''}</span>
          </div>
        </Card>

        <Card>
          <p className="text-[9px] font-bold text-gray-500 uppercase tracking-widest">Battery</p>
        ...
        <Card>
          <p className="text-[9px] font-bold text-gray-500 uppercase tracking-widest">Download</p>
          <p className="mt-1 text-2xl font-bold text-green-500">
            {speed ? formatSpeed(speed.rx_bps) : '\u2014'}
          </p>
          {speed && speed.max_rx_bps > 0 && (
            <p className="mt-1 text-xs text-gray-500">Peak: {formatSpeed(speed.max_rx_bps)}</p>
          )}
        </Card>

        <Card>
          <p className="text-[9px] font-bold text-gray-500 uppercase tracking-widest">Upload</p>
          <p className="mt-1 text-2xl font-bold text-slds-blue">
            {speed ? formatSpeed(speed.tx_bps) : '\u2014'}
          </p>
          {speed && speed.max_tx_bps > 0 && (
            <p className="mt-1 text-xs text-gray-500">Peak: {formatSpeed(speed.max_tx_bps)}</p>
          )}
        </Card>
      </div>
        ...
        <Card title="Signal Detail">
          ...
          {signal && signal.lte_carriers.length > 0 && (
            <div className="mt-3 border-t border-gray-200/60 pt-3">
              <p className="mb-1.5 text-[9px] font-bold text-gray-500 uppercase tracking-widest">LTE ({signal.lte_carriers.length} carrier{signal.lte_carriers.length > 1 ? 's' : ''})</p>
              <div className="flex flex-wrap gap-1.5">
                {signal.lte_carriers.map((c, i) => (
                  <span key={i} className="rounded-lg bg-slds-blue/10 px-2 py-0.5 text-[9px] font-bold text-slds-blueHover border border-slds-blue/30 shadow-sm">{c.band}{c.rsrp != null ? `: ${c.rsrp} dBm` : ''}</span>
                ))}
              </div>
            </div>
          )}
          {signal && signal.nr_carriers.length > 0 && (
            <div className="mt-3 border-t border-gray-200/60 pt-3">
              <p className="mb-1.5 text-[9px] font-bold text-gray-500 uppercase tracking-widest">5G NR ({signal.nr_carriers.length} carrier{signal.nr_carriers.length > 1 ? 's' : ''})</p>
              <div className="flex flex-wrap gap-1.5">
