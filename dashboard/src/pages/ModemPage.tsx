      ...
      <div className="flex gap-2">
        <button onClick={() => apply(0)} disabled={loading || mode === 0}
          className={`rounded-xl px-4 py-2 text-sm font-medium transition-all duration-200 ${
            mode === 0 ? 'bg-green-500/10 text-green-600' : 'bg-slate-50 text-slate-600 hover:bg-slate-50/60'
          } disabled:opacity-40`}>
          Automatic
        </button>
        <button onClick={() => apply(1)} disabled={loading || mode === 1}
          className={`rounded-xl px-4 py-2 text-sm font-medium transition-all duration-200 ${
            mode === 1 ? 'bg-blue-500 text-white' : 'bg-slate-50 text-slate-600 hover:bg-slate-50/60'
          } disabled:opacity-40`}>
          Manual
        </button>
      </div>
    ...
    <div className="space-y-4">
      ...
      <Card title="APN Profiles">
        {loading ? <p className="text-sm text-slate-400">Loading...</p> : profiles.length === 0 ? (
          <p className="text-sm text-slate-400">No manual APN profiles</p>
        ) : (
          <div className="space-y-2">
            {profiles.map(p => (
              <div key={p.profileId} className={`flex items-center justify-between rounded-xl px-3 py-2 ${p.isEnable ? 'bg-white/95 border border-blue-200' : 'bg-slate-50/50'}`}>
                <div>
                ...
                <div className="flex gap-1.5">
                  {!p.isEnable && (
                    <button onClick={() => activateProfile(p.profileId)}
                      className="bg-blue-500 text-white py-3.5 rounded-2xl font-bold shadow-lg shadow-blue-500/20 hover:bg-blue-600 active:scale-[0.98] disabled:opacity-40 transition-all px-4 text-sm px-2 py-1 text-[10px]">Activate</button>
                  )}
        ...
        <Card title="Add APN Profile">
          ...
          <div className="mt-3 flex gap-2">
            <button onClick={addProfile} disabled={!form.name || !form.apn}
              className="bg-blue-500 text-white py-3.5 rounded-2xl font-bold shadow-lg shadow-blue-500/20 hover:bg-blue-600 active:scale-[0.98] disabled:opacity-40 transition-all px-4 text-sm">Add Profile</button>
            <button onClick={() => setAdding(false)}
              className="bg-white border border-slate-200 hover:bg-slate-50 px-3 py-2 rounded-xl font-bold text-slate-500 shadow-sm transition-all active:scale-95 text-sm">Cancel</button>
          </div>
        </Card>
      ) : (
        <button onClick={() => setAdding(true)}
          className="bg-blue-500 text-white py-3.5 rounded-2xl font-bold shadow-lg shadow-blue-500/20 hover:bg-blue-600 active:scale-[0.98] disabled:opacity-40 transition-all px-4 text-sm">+ Add APN Profile</button>
      )}
        ...
        <div className="flex flex-wrap gap-1.5">
          {APN_PRESETS.map(p => (
            <button key={p.name} onClick={() => applyPreset(p)}
              className="rounded-xl bg-slate-50 px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50/60 transition-all duration-200">{p.name}</button>
          ))}
        </div>
      </Card>
    </div>
        ...
        {active === false ? (
          <div className="flex flex-wrap items-center gap-2 pt-1">
            ...
            <button onClick={applyTtl} disabled={loading || !ttlInput}
              className="bg-blue-500 text-white py-3.5 rounded-2xl font-bold shadow-lg shadow-blue-500/20 hover:bg-blue-600 active:scale-[0.98] disabled:opacity-40 transition-all px-4 text-sm py-1.5">
          ...
          <div className="flex flex-wrap items-center gap-y-3 gap-x-4 pt-1">
            <div className="flex items-center gap-2">
              ...
              {ipv6Active && <span className="rounded-xl bg-slate-50/50 px-1.5 py-0.5 text-[10px] text-slate-400">IPv4 + IPv6</span>}
            </div>
            ...
            <div className="flex items-center gap-2 border-l border-slate-200/60 pl-4">
              ...
              <button onClick={applyTtl} disabled={loading}
                className="bg-white border border-slate-200 hover:bg-slate-50 px-3 py-2 rounded-xl font-bold text-slate-500 shadow-sm transition-all active:scale-95 text-sm py-1.5 disabled:opacity-40">
                Update
              </button>
          ...
          <span className="text-sm text-slate-400 pt-1 block">Checking status...</span>
        )}
    ...
    <div className="space-y-4">
      ...
      <Card title="Monthly Usage" action={
        <button onClick={() => { setEditingLimit(true); setLimitGb(String(limit.gb || '')); setResetDay(String(limit.resetDay)) }}
          className="text-xs text-blue-600 hover:text-blue-600 transition-colors">{limit.gb > 0 ? 'Edit Limit' : 'Set Limit'}</button>
      }>
        {editingLimit && (
          <div className="mb-3 flex flex-wrap items-end gap-2 rounded-2xl bg-slate-50/50 p-3">
            <div>
            ...
            <div>
              ...
              <input type="number" min={1} max={28} value={resetDay} onChange={e => setResetDay(e.target.value)}
                className="w-16 px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-400 outline-none text-sm transition-all" />
            </div>
            <button onClick={saveLimit} className="bg-blue-500 text-white py-3.5 rounded-2xl font-bold shadow-lg shadow-blue-500/20 hover:bg-blue-600 active:scale-[0.98] disabled:opacity-40 transition-all px-4 text-sm py-1.5">Save</button>
            <button onClick={() => setEditingLimit(false)} className="bg-white border border-slate-200 hover:bg-slate-50 px-3 py-2 rounded-xl font-bold text-slate-500 shadow-sm transition-all active:scale-95 text-sm py-1.5">Cancel</button>
          </div>
        )}
        ...
        {usage && limit.gb > 0 && (
          <div className="mb-4">
            ...
            <div className="h-2 rounded-full bg-slate-50 overflow-hidden">
              <div className="h-full rounded-full bg-blue-500 transition-all duration-500" style={{
                width: `${usagePct}%`,
              }} />
            </div>
            <p className="mt-1 text-xs text-slate-400">Resets on day {limit.resetDay} of each month</p>
          </div>
        )}
              ...
              <tbody className="divide-y divide-slate-100/60">
                {periods.map(({ label, data }) => (
                  <tr key={label}>
                    ...
                    <td className="py-2 pr-4 text-right text-blue-600">{formatBytes(data.tx_bytes)}</td>
                    <td className="py-2 text-right font-medium text-slate-800">{formatBytes(data.rx_bytes + data.tx_bytes)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {periods.map(({ label, data }) => {
            ...
            return (
              <div key={label} className="mb-4 last:mb-0">
                <p className="mb-1.5 text-[9px] font-bold text-slate-400 uppercase tracking-widest">{label}</p>
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="w-8 text-[10px] text-blue-600">UL</span>
                    <div className="flex-1 h-4 rounded bg-slate-50 overflow-hidden">
                      <div className="h-full rounded bg-blue-500 flex items-center px-1.5 text-[9px] font-medium text-white transition-all"
...
export default function ModemPage() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-3xl font-bold text-slate-800">Modem</h1>

      <div className="bg-slate-50/50 rounded-2xl p-1 flex gap-1 border border-slate-200/50 w-fit">
        {([
          ['apn', 'APN'],
          ['data', 'Data Usage'],
          ['ttl', 'TTL'],
          ...
          <button key={id} onClick={() => setTab(id)}
            className={`rounded-xl px-3 py-1.5 text-sm font-medium transition-all duration-200 ${
              tab === id ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-400 hover:text-slate-600'
            }`}>
            {label}
          </button>
        ))}
