...
export default function MetricsPage() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-3xl font-bold text-slate-800">Metrics</h1>
            ...
            <div className="space-y-3">
              <div>
                <div className="mb-1 flex justify-between text-sm">
                  <span className="font-medium text-slate-800">{battery.capacity}%</span>
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                <div>
                  ...
                  <p className="font-medium text-slate-800">{(battery.power_mw / 1000).toFixed(2)} W</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Voltage</p>
                  <p className="text-slate-800">{(battery.voltage_mv / 1000).toFixed(3)} V</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Current</p>
                  <p className="text-slate-800">{battery.current_ma} mA</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Charge Type</p>
                  <p className="text-slate-800">{battery.charge_type || '—'}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-slate-800">{formatTime(battery.status === 'Charging' ? battery.time_to_full_secs : battery.time_to_empty_secs)}</p>
                </div>
              </div>
            </div>
          ) : (
          ...
          {battery ? (
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="text-slate-800">{battery.health}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Capacity</span>
                <span className="text-slate-800">{battery.charge_full_mah} / {battery.charge_full_design_mah} mAh</span>
              </div>
              {batteryHealth != null && (
                <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-slate-800">{battery.cycle_count}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">OCV</span>
                <span className="text-slate-800">{(battery.voltage_ocv_mv / 1000).toFixed(3)} V</span>
              </div>
            </div>
          ) : (
            <p className="text-sm text-slate-400">Loading...</p>
