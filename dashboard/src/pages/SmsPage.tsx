  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-slate-800">SMS</h1>
        <button
          ...
          <form onSubmit={send} className="space-y-3">
            ...
            <div>
              ...
              <textarea value={text} onChange={e => setText(e.target.value)} required rows={4}
                className="w-full resize-none px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-400 outline-none text-sm transition-all"
                placeholder="Type a message…" />
              <p className="mt-0.5 text-right text-xs text-slate-400">{text.length}/160</p>
            </div>
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
        ...
        <Card
          ...
          action={
            <div className="flex gap-2">
              <button onClick={() => setBox(BOX_INBOX)} className={`text-xs ${box === BOX_INBOX ? 'text-blue-600' : 'text-slate-400 hover:text-slate-800 transition-colors'}`}>Inbox</button>
              <button onClick={() => setBox(BOX_SENT)} className={`text-xs ${box === BOX_SENT ? 'text-blue-600' : 'text-slate-400 hover:text-slate-800 transition-colors'}`}>Sent</button>
            </div>
          }
        >
          {loading ? (
            ...
            <ul className="divide-y divide-slate-100/60 -mx-4">
              {messages.map(m => (
                <li key={m.id}
                  onClick={() => openMsg(m)}
                  className={`cursor-pointer px-4 py-3 transition-colors hover:bg-slate-50/60 ${selected?.id === m.id ? 'bg-blue-50' : ''}`}
                >
                  <div className="flex items-start justify-between gap-2">
                    <p className={`text-sm truncate ${!m.read ? 'font-semibold text-slate-800' : 'text-slate-600'}`}>
                      {m.from ?? '—'}
                    </p>
                  ...
                  <p className="mt-0.5 text-xs text-slate-400">{formatDate(m.date)}</p>
                </li>
              ))}
            </ul>
          )}
          ...
          {selected ? (
            <div className="space-y-3">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-sm font-medium text-slate-800">From: {selected.from ?? '—'}</p>
              ...
              <div className="rounded-2xl bg-slate-50/50 p-4">
                <p className="whitespace-pre-wrap text-sm text-slate-800">{selected.text}</p>
              </div>
              <button
            ...
            <p className="text-sm text-slate-400">No message selected</p>
          )}
        </Card>
      </div>
    </div>
