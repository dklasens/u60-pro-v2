...
const BTN_PRIMARY = 'bg-blue-500 text-white py-3.5 rounded-2xl font-bold shadow-lg shadow-blue-500/20 hover:bg-blue-600 active:scale-[0.98] disabled:opacity-40 transition-all px-4 text-sm'

function Input({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
    <div>
      <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5 block">{label}</label>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        <div className="flex gap-2">
          <button onClick={() => setDns(d => ({ ...d, primary: '1.1.1.1', secondary: '1.0.0.1', ipv6_primary: '2606:4700:4700::1111', ipv6_secondary: '2606:4700:4700::1001' }))}
            className="text-xs text-blue-600 hover:text-blue-700 transition-colors font-bold">Cloudflare</button>
          <button onClick={() => setDns(d => ({ ...d, primary: '8.8.8.8', secondary: '8.8.4.4', ipv6_primary: '2001:4860:4860::8888', ipv6_secondary: '2001:4860:4860::8844' }))}
            className="text-xs text-blue-600 hover:text-blue-700 transition-colors font-bold">Google</button>
          <button onClick={() => setDns(d => ({ ...d, primary: '9.9.9.9', secondary: '149.112.112.112', ipv6_primary: '2620:fe::fe', ipv6_secondary: '2620:fe::9' }))}
            className="text-xs text-blue-600 hover:text-blue-700 transition-colors font-bold">Quad9</button>
        </div>
      </div>
    </Card>
  )
...
export default function RouterPage() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-3xl font-bold text-slate-800">Router</h1>

      <div className="bg-slate-50/50 rounded-2xl p-1 flex gap-1 w-fit border border-slate-200/50">
        {([
          ['lan', 'LAN / DHCP'],
          ['dns', 'DNS'],
